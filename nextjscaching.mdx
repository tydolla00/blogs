...
<InfoBlock type="info">
  ...
  experienced React dev wrote an amazing blog post about it! Check it out here.
  <BlueLink href="https://www.joshwcomeau.com/react/server-components/">
...
is stored in memory. On the first request this will be a cache miss. After
receiving the response, the response will be stored in memory (cached). Subsequent
requests to the same endpoint will be a cache hit and will never hit
the endpoint. There are four key points of caching in Next.
